...
goals = np.load('/home/viki/trajopt_utils/finalstart3SameTestFirst.txt.npy')
inits = np.load('/home/viki/trajopt_utils/finalinit3SameTestFirst.txt.npy')

# should test this!
initializations = []
for i in range(len(inits)-1):
	initializations.append(inits[i][::-1])
...
# goal = [-.6,0,-1.5,-1.5,0,-.6,0] -> for the tabletop planning problem
# goal = [-0.8, -0.3,  0.1, -0.19999999,  0., -0.20000001,  0.10000001] -> for the cluttered init problem
robot.SetActiveDOFValues(startDOF)
errors, trajs, starts = planToJointPos2(env, TPlanner, goals, initializations)